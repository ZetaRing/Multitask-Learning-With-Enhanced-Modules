...
def train():
  ...
  i = 0
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = modules.conv_module(image_shaped_input, FLAGS.filt, [5,5], geopath[i,j], 1,  'layer'+str(i+1)+"_"+str(j+1))
  net=np.sum(map(lambda (a,b):a*b[0], zip(layer_modules_list , sum_weights_list[0])))/FLAGS.M;
    # res-fire layer
  i = 1
  for j in range(FLAGS.M):
  ...
  net=np.sum(map(lambda (a,b):a*b[0], zip(layer_modules_list , sum_weights_list[1])))/FLAGS.M;
    # dimensionality_reduction layer
  i = 2
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = modules.Dimensionality_reduction_module(net, 10, geopath[i,j], 'layer'+str(i+1)+"_"+str(j+1))
  net=np.sum(map(lambda (a,b):a*b[0], zip(layer_modules_list , sum_weights_list[2])))/FLAGS.M;
    # conv layer
  i = 3
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = modules.conv_module(image_shaped_input, FLAGS.filt, [5,5], geopath[i,j], 1,  'layer'+str(i+1)+"_"+str(j+1))
  net=np.sum(map(lambda (a,b):a*b[0], zip(layer_modules_list , sum_weights_list[3])))/FLAGS.M;
  # output layer
    # reshape
  _shape = net.shape[1:]
  _length = 1
...
if __name__ == '__main__':
  ...
  parser.add_argument('--svhn_data_dir', type=str, default='/home/petch/Desktop/pathnet/data_set/svhn_data',
