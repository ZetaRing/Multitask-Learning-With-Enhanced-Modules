...
def train():
  ...
  i = 0
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = modules.conv_module(sum_weights_list[i,j][1] *image_shaped_input, FLAGS.filt, [11,11], geopath[i,j], 1,  'layer'+str(i+1)+"_"+str(j+1))
  ...
  i = 4
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = modules.module(sum_weights_list[i,j][1] *net, FLAGS.full_connection_filt, geopath[i,j], 'layer'+str(i+1)+"_"+str(j+1))
  net=np.sum(map(lambda (a,b):a*b[0], zip(layer_modules_list , sum_weights_list[i])))/ FLAGS.M
  ...
  for i in range(len(task1_optimal_path)):
    for j in range(len(task1_optimal_path[0])):
      if(task1_optimal_path[i,j]==task2_optimal_path[i,j])&(task1_optimal_path[i,j]==1.0):
        overlap+=1;
  print("ImageNet,TASK1:"+str(acc_task1)+",TASK2:"+str(acc_task2)+", Overlap:"+str(overlap))
